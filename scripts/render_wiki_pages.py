...
"""Stage 7: render Wikipedia-style pages from the hidden world.
    ...
    python scripts/render_wiki_pages.py                 # full re-render (entities + events)
    python scripts/render_wiki_pages.py --incremental    # only entities/events w/o a page
    python scripts/render_wiki_pages.py --incremental --build
    python scripts/render_wiki_pages.py --no-events       # entity pages only
"""
...
def main() -> None:
    ...
    parser.add_argument("--incremental", action="store_true",
                        help="Render only entities/events that do not yet have a page.")
    parser.add_argument("--no-events", action="store_true",
                        help="Skip rendering a page per historical event.")
    ...
    if not args.no_events:
        render_event_pages(cfg, llm, incremental=args.incremental)
